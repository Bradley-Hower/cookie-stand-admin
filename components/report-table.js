export default function ReportTable(props) {
  if (props.reports.length === 0) {
    return <h2 className="flex justify-center py-12 text-2xl text-gray-600r">No Cookie Stands Available</h2>
  }
...
function ReportRow({ report }){
  ...
  return (
    <tr className="odd:bg-green-400">
      {values.map((value, e) => <td className="pl-4 border border-green-900" key={e}>{value}</td>)}
